...
template<typename T>
class TransImplSM: public TransImplStrategy<T>
{
public:
...
template<typename T>
__global__ void transSMKernel(T const *input, T *output,
...
template<typename T>
void TransImplSM<T>::launchKernel(const T *input,
                                  T *output,
                                  ...
                                  unsigned int num_cols_input) const
{
    ...
    transSMKernel<<<grid_size, block_size>>>(input, output, num_rows_input, num_cols_input);
    checkErrorKernel("transSMKernel", true);
}

}// BPNV namespace.

#endif //TRANS_IMPL_SM_CUH
