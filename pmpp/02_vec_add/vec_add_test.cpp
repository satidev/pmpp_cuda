...
TEST(vecAddTest, exceptThrowSizeMismatch) {
    EXPECT_THROW(PMPP::vecAdd(std::vector{1.0f, 3.0f}, std::vector{2.0f}),
                 std::invalid_argument);
}

TEST(vecAddTest, singleElemVecAdd) {
    ASSERT_THAT(PMPP::vecAdd(std::vector{1.0f}, std::vector{2.0f}),
                testing::ElementsAre(3.0f));
}

TEST(vecAddTest, multiElemVecAdd) {
    ASSERT_THAT(PMPP::vecAdd(std::vector{1.0f, 2.0f, -4.0f},
...
TEST(vecAddTest, largeVecAdd) {
    ...
    ASSERT_THAT(PMPP::vecAdd(vec_1, vec_2), testing::ContainerEq(res_exp));
}