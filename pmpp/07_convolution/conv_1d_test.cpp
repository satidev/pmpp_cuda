...
TEST(conv1DTest, exceptionIsThrownSinceDataSizeIsZero)
{
    ...
    EXPECT_THROW(PMPP::CUDA::conv1D(data, filter), std::invalid_argument);
}
...
TEST(conv1DTest, exceptionIsNotThrownSinceFilterSizeIsOdd)
{
    auto const data = std::vector<float>{1.0f, 2.0f, 3.0f, 4.0f};
    auto const filter = std::vector<float>{1.0f, 2.0f, 3.0f};
    EXPECT_NO_THROW(PMPP::CUDA::conv1D(data, filter));
}
...
TEST(conv1DTest, exceptionIsThrownSinceFilterSizeExceedsMaxFilterSizeWhenConstMemIsUsed)
{
    ...
    EXPECT_THROW(PMPP::CUDA::conv1D(data, filter, true), std::invalid_argument);
}
...
TEST(conv1DTest, exceptionIsNotThrownForSharedMem)
{
    ...
    EXPECT_NO_THROW(PMPP::CUDA::conv1D(data, filter, true, true));
}
...
TEST(conv1DTest, inputAndOutputDataSizeIsSame)
{
    auto const data = std::vector<float>{1.0f, 2.0f, 3.0f, 4.0f};
    auto const filter = std::vector<float>{1.0f, 2.0f, 3.0f};
    auto const res = PMPP::CUDA::conv1D(data, filter);
...
TEST(conv1DTest, correctConv1DResultForIdentityKernel)
{
    ...
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, false, false), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, true, false), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, false, true), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, true, true), ::testing::ContainerEq(data));
}
...
TEST(conv1DTest, correctConv1DResultForSymmetricFilterKernel)
{
    ...
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, false), ::testing::ContainerEq(expected));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, true), ::testing::ContainerEq(expected));
...
TEST(conv1DTest, correctConv1DResultForIdentityKernelLargeArray)
{
    ...
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, false), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, true), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, false, true), ::testing::ContainerEq(data));
    EXPECT_THAT(PMPP::CUDA::conv1D(data, filter, true, true), ::testing::ContainerEq(data));
}
