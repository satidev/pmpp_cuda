...
namespace PMPP::CUDA
{
                      ...
                      ReductionStrategy strategy)
    {
        ...
        else
        {
            ...
            {
                auto const [grid_size, block_size] = Detail::execConfig(num_elems, strategy);
                sumParallelSimpleMinDivSharedMultBlock<<<grid_size, block_size, block_size * sizeof(float)>>>(data_dev.data(), sum_dev.data());
            }
            break;
            case ReductionStrategy::SIMPLE_MIN_DIV_SHARED_MULT_BLOCKS_COARSE:
            {
                auto const [grid_size, block_size] = Detail::execConfig(num_elems, strategy);
                sumParallelSimpleMinDivSharedMultBlockCoarse<<<grid_size, block_size, block_size * sizeof(float)>>>(data_dev.data(), sum_dev.data());
            }
            break;
            }
